	...
	for (int waitT = 0; waitT < t; ++waitT)
	{
		...
		for (int moveT = startAttackTick; moveT < t - waitT; ++moveT)
		{
				...
				curJumpCollisionT, collision_ball_entity, collideBallEntity))
			{
				...
				const auto bec = BallEntityContainer(collision_ball_entity.value(), curCollisionT, isGoal, goalTime, collisionsCount, collideBallEntity, false);
	...
	int position)// -1 - защ, 0 - пз, 1 - нап
{
	...
	bool needUseDefenceNitro = position == -1 && me.nitro_amount > 0 && (_lastMyCollisionTick == -1 || _isMeGoalPossible)
			...
			{				
					...
					re.Velocity.Y < 0) //летим вниз после коллизии
				{
					...
					auto bec = BallEntityContainer(resBes[0], collisionTime, 
						isGoal, goalTime,collisions_count, collideBe, true);
		...
		for (int t = 0; t < finalTick; ++t)
		{
			...
			for (int moveT = 0; moveT <= t; ++moveT)
			{
				...
				if (isCollision && 					 
					...
					(!_isGoalPossible || (collisionTime + goalTime) * Constants::Rules.TICKS_PER_SECOND < _meGoalScoringTick))//уменьшаем время забития TODO
				{
	...
	if (position >= 0 && me.z > 0 && !_isGoalPossible && _isNoCollisionGoalPossible && me.nitro_amount > 0)
	{
		bool isGot = false;
		BallEntityContainer nitroBestBec;
		...
		for (auto & oppCt:_oppBallCollisionTicks)
		{
			...
			for (int t = 0; t < collisionTick; ++t)
			{
				...
				for (int moveT = 0; moveT <= t; ++moveT)
				{
					...
					if (isCollision &&
						collisionTime * Constants::Rules.TICKS_PER_SECOND < _meGoalScoringTick < collisionTick)//бьем по роботу до коллизи с мячом
					{
						...
						{
							isGot = true;
							auto bec = BallEntityContainer(_ballEntities[0], collisionTime, true, goalTime, 0, BallEntity(), true);
							nitroBestBec = bec;
							isOkBestBecP = true;
				...
				//for (auto moveT = 0; moveT < t; ++moveT)//двигаемся как можно дольше
				//{
	...
	bool& isDefenderSavedPointOk, BallEntityContainer& bestBecP, int& bestWaitT, int& bestMoveT, int position)
{
	...
	for (int t = startAttackTick; t <= startAttackTick + BallMoveTicks; ++t)
	{
		...
		else
		{
			...
			if (t == 0)
			{
				...
				if (IsOkPosToJump(robotEntity, 0, jumpCollisionT, curBallEntity, goalTime, collisionsCount))
				{
					BallEntityContainer bec = BallEntityContainer(
						curBallEntity.value(), jumpCollisionT.value(), true, goalTime, collisionsCount, BallEntity(), false);
					bestWaitT = 0;
					bestMoveT = 0;
					isResOk = true;
			...
			BallEntityContainer bec = BallEntityContainer(curBallEntity.value(), curCollisionT, true, goalTime, collisionsCount, BallEntity(), false);
	...
	RobotEntity & re, int oppRobotId, int startTick, double & collisionTime, bool isJump)
{
	...
	while (_robotEntities.count(t + 1) > 0 && jumpT + 1 < 60)
	{
		...
		if (curRe.Position.Y - reY > collisionDist)//еще не достигли нужной высоты
		{
			/*if ((reX - curBe.Position.X) * startDx < 0 ||
				(reZ - curBe.Position.Z) * startDz < 0)
				return false;*/
		...
		if (reY - curRe.Position.Y > collisionDist)//пролетели выше робота
		{
			return false;
		}
