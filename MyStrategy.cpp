...
void MyStrategy::InitJumpingRobotAction(const Robot& robot, const Ball& ball)
{
	...
	{
		//включаем, чтобы ударить по воротам
		...
		{
			//включаем, чтобы ударить по противнику
			...
			for (auto & oppCt : _oppBallCollisionTicks)
			{
				...
				if (isCollision &&
					collisionTime * Constants::Rules.TICKS_PER_SECOND < oppCollisionTick - collisionTickDelta)//бьем по роботу до коллизи с мячом
				{
	...
	//{
	//	//включаем, чтобы просто ударить по мячу раньше противника
...
{	
	...
	while (_ballEntities.count(t+1) > 0 && jumpT + 1 < 100)
	{
		...
		if (re.Position.Y - curBe.Position.Y > collisionDist)//пролетели выше мяча
		{
			return false;
		}
